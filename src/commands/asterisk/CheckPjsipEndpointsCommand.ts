...
import { CommandHandler, CommandMetadata } from '../../application/command/CommandRegistry'
...
export class CheckPjsipEndpointsCommand implements CommandHandler {
  ...
  async execute(): Promise<unknown> {
    ...
    try {
      ...
      if (!endpoints || endpoints.length === 0) {
        console.log('No PJSIP endpoints found or PJSIPShowEndpoints did not return EndpointList events.')
        return {
      ...
      const registeredCount = processedEndpoints.filter((ep) => ep.isRegistered).length

      console.log(`Retrieved ${endpoints.length} PJSIP endpoints, ${registeredCount} registered`)
  ...
  private processEndpoints(endpoints: ManagerEvent[]): EndpointResult[] {
    return endpoints.map((endpointEvent) => {
      const objectName = endpointEvent.objectname || endpointEvent.endpointname || 'N/A'
      ...
      if (aor) {
        if (
          contactStatus &&
          (contactStatus.toLowerCase() === 'reachable' || contactStatus.toLowerCase() === 'registered')
        ) {
          isRegistered = true
  ...
  getMetadata(): CommandMetadata {
    return {
      ...
      description: 'Retrieves all configured PJSIP endpoints from Asterisk and checks their registration status',
      domain: 'asterisk',
      parameters: [],
    }
  }
