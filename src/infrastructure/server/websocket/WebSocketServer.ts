...
function isAvailableCommandsMessage(
  message: any,
): message is AvailableCommandsMessage {
  return (
    message &&
    typeof message === "object" &&
    message.type === "getAvailableCommands"
  );
}
      ...
      ws.on("message", async (message: WebSocket.RawData) => {
        ...
        try {
          ...
          if (isAvailableCommandsMessage(parsedMessage)) {
            ...
            "command" in parsedMessage &&
            typeof parsedMessage.command === "string"
          ) {
            // Handle command execution
            const result = await this.commandService.handleCommand(
            ...
            ws.send(
              JSON.stringify({
                status: "error",
                message:
                  'Invalid message format. Expected "command" field or "type" field.',
              }),
            );
          }
        } catch (error) {
